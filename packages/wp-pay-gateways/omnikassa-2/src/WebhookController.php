	...
	public function rest_api_init() {
		...
		\register_rest_route(
			...
			[
				'args'                => [
					'id' => [
						'description' => \__( 'Unique identifier for the gateway configuration post.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
						'type'        => 'integer',
					],
				],
	...
	public function rest_api_omnikassa_2_webhook_item( WP_REST_Request $request ) {
		...
		try {
			...
			return new \WP_Error(
				...
				\__( 'Invalid Rabo Smart Pay notification.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
				[
					'notification' => $json,
					'status'       => 400,
				]
		...
		if ( ! \is_numeric( $id ) ) {
			return new \WP_Error(
				...
				\__( 'No gateway ID given in `id` parameter.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' )
			);
		}
		...
		if ( ! $gateway instanceof Gateway ) {
			...
			return new \WP_Error(
				...
				\__( 'Invalid Rabo Smart Pay gateway.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
				[
					'id'     => $id,
					'status' => 400,
				]
