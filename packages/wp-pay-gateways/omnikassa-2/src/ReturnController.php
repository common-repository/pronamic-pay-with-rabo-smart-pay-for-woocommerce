...
final class ReturnController {
	...
	public function rest_api_init() {
		\register_rest_route(
			...
			[
				'args'                => [
					'order_id'   => [
						'description' => \__( 'OmniKassa order ID.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
						'type'        => 'string',
					],
					'payment_id' => [
						'description' => \__( 'Unique identifier for the payment.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
						'type'        => 'integer',
					],
					'signature'  => [
						'description' => \__( 'OmniKassa signature.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
						'type'        => 'string',
					],
					'status'     => [
						'description' => \__( 'OmniKassa order status.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
						'type'        => 'string',
					],
				],
		...
		if ( null === $payment ) {
			throw new \InvalidArgumentException(
				\sprintf(
					...
					\esc_html__( 'No payment found by `payment_id` variable: %s.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
	...
	public function rest_api_return( WP_REST_Request $request ) {
		try {
			...
			$note .= '<p>';
			$note .= \__( 'Rabo Smart Pay return URL requested:', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' );
			$note .= '</p>';
