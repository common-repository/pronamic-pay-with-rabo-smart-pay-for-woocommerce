	...
	private function get_payment_status_update_note( $old_status, $new_status ) {
		...
		if ( null === $old_status ) {
			return sprintf(
				...
				__( 'Payment created with status "%1$s".', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
		...
		return sprintf(
			...
			__( 'Payment status changed from "%1$s" to "%2$s".', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
	...
	public function rest_api_init() {
		\register_rest_route(
			...
			[
				...
				'args'                => [
					'payment_id' => [
						'description' => __( 'Payment ID.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
						'type'        => 'integer',
					],
				],
			]
		...
		if ( null === $payment ) {
			return new \WP_Error(
				...
				\sprintf(
					...
					\__( 'Could not find payment with ID `%s`.', 'pronamic-pay-with-rabo-smart-pay-for-woocommerce' ),
					$payment_id
				),
				$payment_id
			);
